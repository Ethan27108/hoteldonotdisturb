...
class MaidLoginView(APIView):
    ...
    def post(self, request):
        ...
        if user and Maid.objects.filter(user=user).exists():
            token, created = Token.objects.get_or_create(user=user)
            return Response({"token": token.key, "role": "maid"}, status=status.HTTP_200_OK)
        return Response({"error": "Invalid credentials or not a Maid"}, status=status.HTTP_401_UNAUTHORIZED)
